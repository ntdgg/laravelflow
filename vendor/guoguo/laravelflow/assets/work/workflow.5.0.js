...
var laravelflow = {
	...
	Init : function(processData) {
		...
		$.each(processData.list, function(i,row) {
			...
			$(nodeDiv).attr("id",nodeId)
				...
				.html('<span class="process-flag" style="text-align: left;"><img src="'+laravelflow.Ico()+'" width=18px alt=""></span>&nbsp;' +row.process_name + '<br/><img alt="" src="'+laravelflow.Ico(1)+'" width=18px>&nbsp;' +row.mode + '<br/><img alt="" src="'+laravelflow.Ico(2)+'" width=18px>&nbsp;' +row.name + '' );
		...
		$(".process-step").live('click',function(){
			...
			laravelflow.DClick($(this).attr("process_id"))
		})
	...
	show : function(processData) {
		...
		$.each(processData.list, function(i,row) {
			...
			$(nodeDiv).attr("id",nodeId)
				...
				.attr("process_id",row.id)
				.addClass("process-step wf_btn")
				.html('<span class="process-flag" style="text-align: left;"><img src="'+laravelflow.Ico()+'" width=18px></span>&nbsp;' +row.process_name + '<br/><img src="'+laravelflow.Ico(1)+'" width=18px>&nbsp;' +row.mode + '<br/><img src="'+laravelflow.Ico(2)+'" width=18px>&nbsp;' +row.name + '' );
			_this.append(nodeDiv);
			lastProcessId = row.id;
		});
		initEndPoints();
	...
	DClick : function(id) {
		...
		laravelflow.lopen("属性设计",url,50,60);
	...
	cc :function(id){
		...
		laravelflow.sPost(Url,{"id":id},true);
	},
	Api : function(Action) {
		var reload = false;
		switch(Action) {
			case 'save':
				var PostData = {"flow_id":the_flow_id,"process_info":laravelflow.GetJProcessData()};//获取到步骤信息
				break;
			case 'delAll':
			...
			case 'att':
				...
				break;
			case 'add':
				laravelflow.sPost(Server_Url+'?act=save',{"flow_id":the_flow_id,"process_info":laravelflow.GetJProcessData()},false);
				var PostData = {"flow_id":the_flow_id};
				reload = true;
				break;
			case 'check':
			...
			case 'Doc':
				...
				break;
			default:
				window.open("//gitee.com/ntdgg/laravelflow");

		}
		var Url = Server_Url+'?act='+Action;
		laravelflow.sPost(Url,PostData,reload);
	},
	wfconfirm : function(url,data,msg) {
		layer.confirm(msg, {
			btn: ['执行','取消'] //按钮
		}, function(){
			laravelflow.sPost(url,data);
		}, function(){
			layer.msg('取消操作', {
				time: 2000, //20s后自动关闭
			});
